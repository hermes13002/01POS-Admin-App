...
class RestockSuggestionModel {
  ...
  factory RestockSuggestionModel.fromJson(Map<String, dynamic> json) {
    return RestockSuggestionModel(
      ...
      averageDailySales: double.tryParse(json['average_daily_sales']?.toString() ?? '0.0') ?? 0.0,
      lowStockLimit: double.tryParse(json['low_stock_limit']?.toString() ?? '0.0') ?? 0.0,
      stockOutDays: json['stock_out_days'] != null ? int.tryParse(json['stock_out_days'].toString()) : null,
      stockOutDate: json['stock_out_date']?.toString(),
      suggestedReorderQty: double.tryParse(json['suggested_reorder_qty']?.toString() ?? '0.0') ?? 0.0,
